...
def cross_validation(ts_dfs, model, ITR=5000, 
                     ...
                     end=None, #pd.datetime(2017, 7, 10), 
    ...
    '''
    if end is None:
        end = ts_dfs[0]["ds"].max() # use the whole timeseries
    
    predictions = []
    metrics = []
    while start < end:
        print("[+] Validation: train [--%s], test [%s-%s]" % (start, start, start+length))

        #  split history (train) / future (test)
        ts_data = []
        for df in ts_dfs: 
            history, future, y_scale = split_train_test(df, start, start+length)
            ts_data.append({
